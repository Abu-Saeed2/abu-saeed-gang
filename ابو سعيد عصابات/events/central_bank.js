...
module.exports = async (client, interaction) => {
  ...
  if (
    ...
    interaction.values[0] === "central_bank"
  ) {
    const modal = new ModalBuilder()
      .setCustomId("central_bank_modal")
      .setTitle("معلومات سرقة البنك المركزي")
      .addComponents(
  ...
  /* ================= MODAL ================= */
  if (interaction.isModalSubmit() && interaction.customId === "central_bank_modal") {
    robberyInfo.set(uid, {
    ...
    return interaction.reply({
      embeds: [
        new EmbedBuilder()
          .setColor("DarkBlue")
          .setTitle("🏦 Central Bank Heist"),
      ],
      components: [
        new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId("central_bank_start")
            .setLabel("استكمال")
  ...
  /* ================= STEP 1 — ألوان ================= */
  if (interaction.isButton() && interaction.customId === "central_bank_start") {
    state.step = 1;
    state.reds = pickReds();
    state.clicks = [];
  ...
  if (state.step === 1 && interaction.customId.startsWith("cb1_")) {
    ...
    setTimeout(async () => {
      const rows = [
        new ActionRowBuilder().addComponents(...[1,2,3,4,5].map(n =>
          new ButtonBuilder().setCustomId(`cb2_${n}`).setLabel(`${n}`).setStyle(ButtonStyle.Secondary)
        )),
  ...
  if (state.step === 2 && interaction.customId.startsWith("cb2_")) {
  ...
  if (state.step === 3 && interaction.customId.startsWith("cb3_")) {
    const idx = parseInt(interaction.customId.split("_")[1]);
    if (!state.clicks.includes(idx)) state.clicks.push(idx);
    ...
    setTimeout(async () => {
      const rows = [
        new ActionRowBuilder().addComponents(...[1,2,3,4,5].map(n =>
          new ButtonBuilder().setCustomId(`cb4_${n}`).setLabel(`${n}`).setStyle(ButtonStyle.Secondary)
        )),
        new ActionRowBuilder().addComponents(...[6,7,8,9,0].map(n =>
          new ButtonBuilder().setCustomId(`cb4_${n}`).setLabel(`${n}`).setStyle(ButtonStyle.Secondary)
        )),
  ...
  if (state.step === 4 && interaction.customId.startsWith("cb4_")) {
    const digit = interaction.customId.split("_")[1];
    const s = codeMap.get(uid);
    if (!s) return;
    ...
    if (channel && info) {
      channel.send({
        embeds:[
          new EmbedBuilder()
            .setColor("Red")
            .setTitle("🚨 بلاغ سرقة البنك المركزي")
            .addFields(
    ...
    return interaction.update({
      embeds:[
        new EmbedBuilder()
          .setColor("Gold")
          .setTitle("✅ تمت سرقة البنك المركزي بنجاح"),
      ],
      components:[],
      ephemeral:true,
    });
